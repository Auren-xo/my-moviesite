...
function TvShows() {
  ...
  const apiKey = process.env.REACT_APP_TMDB_API_KEY || "77f876d7cdee43f24835c3b9cdf053d8";
  ...
  useEffect(() => {
    ...
    fetchTv();
  }, [input, apiBase, apiKey]);

  const handlePlay = async (show) => {
    try {
      const res = await axios.get(`https://api.themoviedb.org/3/tv/${show.id}/videos`, {
  ...
  return (
    <div className={toggle ? "mainBgColor" : "secondaryBgColor"}>
      <div className="movies-container">
        ...
        {showTrailer && (
          <TrailerPlayer trailerUrl={trailerUrl} onClose={() => setShowTrailer(false)} />
        )}
      </div>
    </div>
  );
